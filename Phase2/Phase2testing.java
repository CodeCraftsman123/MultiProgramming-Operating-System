...
public class Phase2testing
{
    ...
    private static void TERMINATE(int number)
    {
        ...
        if(number == 0 || number == 2 || number == 3)
            Phase2testing.SIMULATION();//1 cycle used for termination.
        try
        {
    ...
    private static int ADDRESSMAP(String VA)
    {
        if(flagForIC)//For IC
        {
            ...
            if (isNumeric(VA))
            {
                ...
                {
                    PI = 3;
                    Phase2testing.MOS();//Invalid page fault
            ...
            {
                PI = 2;
                Phase2testing.MOS();//Operand Error
                return Integer.MIN_VALUE;
            }
        }
            ...
            if(isNumeric(VA))
            {
                ...
                catch(Exception e)//Page fault has occurred, we got **(that's why Integer.parseInt())gave an error
                {
                    if(opcode.equals("GD") || opcode.equals("SR"))//Valid Page Fault
                    {
                        ...
                        Phase2testing.UPDATEPAGETABLE(frameNumber);
                        frameNo = Integer.parseInt(""+M[genericPageTablePtr][2]+M[genericPageTablePtr][3]);
                        RA = ((frameNo * 10) + (Integer.parseInt(VA) % 10));
                        return RA;
                    }
    ...
    private static void EXECUTEUSERPROGRAM()
    {
        while(true)
        {
            ...
            flagForIC = true;
            IC = Phase2testing.ADDRESSMAP(Integer.toString(IC));
            flagForIC = false;
            ...
            for(int i = 0;i < 10;i++)
            {
                ...
                if(!opcode.equals("H"))
                {
                    RA = Phase2testing.ADDRESSMAP(operand);
                    if (Phase2testing.errorHasOccurredOrNot(RA))
                        return;
                }
    ...
    {
        IC = 0;
        Phase2testing.EXECUTEUSERPROGRAM();
    }
    ...
    private static void LOAD()
    {
        while(scanner.hasNextLine())
        {
            ...
            if (first4CharOfInputLine.equals("$AMJ"))
            {
                ...
                pageTableBasePtr = (Phase2testing.ALLOCATE() * 10);
                Phase2testing.INITIALIZEPAGETABLE();
                flag1 = true;
            }
            else if (first4CharOfInputLine.equals("$DTA"))
            {
                Phase2testing.STARTEXECUTION();
